...
class Learner(object):
    ...
    def LoadQvalues(self, path="sarsa.json"):
        with open(path, "r") as f:
            qvalues = json.load(f)
        return qvalues

    def SaveQvalues(self, path="sarsa.json"):
    ...
    def act(self, snake, food):
        ...
        if self.next_action == '':
            action_key = random.choices(list(self.actions.keys()))[0] # 0,1,2,3 random choose one
        else:
            # # Epsilon greedy
    ...
    def next_act(self, snake, food):
        next_state = self._GetState(snake, food)
        rand = random.uniform(0,1)
        if rand < self.epsilon:
            action_key = random.choices(list(self.actions.keys()))[0] # 0,1,2,3 random choose one
        else:
            state_scores = self.qvalues[self._GetStateStr(next_state)]
            action_key = state_scores.index(max(state_scores))
        self.next_action = action_key

    def UpdateQValues(self, reason):
        history = self.history[::-1]
        for i, h in enumerate(history[:-1]): # i is the enumerate number, h is the dictionary element in history
    ...
    def _GetState(self, snake, food):
        ...
        for sq in sqs:
            if sq[0] < 0 or sq[1] < 0: # off screen left or top
                ...
                surrounding_list.append('1')
            elif sq in snake[:-1]: # part of_GetStateStr tail
                surrounding_list.append('1')
            else:
